import { subscribe, getCv, isDirty, getPreviewLanguage, togglePreviewLanguage } from "./state.js";
...
// Reuse a previously picked folder on startup, without prompting, if the browser
// still grants permission (it may not — File System Access can require a fresh
// user gesture each session, in which case the user just clicks "CV Folder" again).
reuseCvFolder().then(async (handle) => {
  if (!handle) return;
  cvFolder = handle;
